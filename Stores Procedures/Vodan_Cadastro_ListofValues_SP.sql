DELIMITER ;
DROP PROCEDURE IF EXISTS `Vodan_Cadastro_ListofValues_SP`;
DELIMITER $$
CREATE PROCEDURE `Vodan_Cadastro_ListofValues_SP`(input  JSON)
BEGIN

DECLARE DSDescricao VARCHAR(100);
DECLARE IDListType INT;
DECLARE IDListofValues INT;
...
IF liDados > 0 THEN
	...
	WHILE ind < liDados DO
		...
		SET IDListofValues = VODAN_EXTRACT_INT(input,ind, 'SQ_ListofValues');
