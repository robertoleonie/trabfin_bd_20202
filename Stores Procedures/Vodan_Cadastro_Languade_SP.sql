DELIMITER ;
DROP PROCEDURE IF EXISTS `Vodan_Cadastro_Language_SP`;
DELIMITER $$
CREATE PROCEDURE `Vodan_Cadastro_Language_SP`(input  JSON)
BEGIN
...
IF liDados > 0 THEN
	...
	WHILE ind < liDados DO
		...
		SET IDLanguage = VODAN_EXTRACT_INT(input,ind, 'SQ_Language');
		SET DSDescricao = VODAN_EXTRACT_TXT(input,ind,'DS_Descricao');

		IF Acao != 'INCLUIR' AND Acao != 'ALTERAR' AND Acao != 'EXCLUIR'  THEN
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_Language_SP Acao vazia  ';  /* RETORNAR ERRO */
		END IF;
		...
		IF DSDescricao IS NULL AND  Acao = 'INCLUIR' THEN
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_Language_SP DS_Descricao não preenchida  ';  /* RETORNAR ERRO */
		END IF;

		IF DSDescricao IS NOT NULL THEN
			IF DSDescricao = '' THEN
				SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_Language_SP DS_Descricao não preenchida';
			ELSE
				SET liCount = (SELECT COUNT(*) FROM tb_language
						WHERE  DSDescricao =  DSDescricao
						AND languageID != IDLanguage
						);
				IF liCount > 0  AND  Acao = 'INCLUIR' THEN
					SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_Language_SP DS_Descricao já existe';
				END IF;
			END IF;
		END IF;

		IF  Acao != 'INCLUIR' THEN
			SET IDLanguage = (SELECT languageID FROM tb_language
						WHERE languageID = IDLanguage);
			IF IDLanguage IS NULL THEN
				SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_Language_SP SQ_Language não localizado na base';
			END IF;
		END IF;
