DELIMITER ;
DROP PROCEDURE IF EXISTS `Vodan_Cadastro_ListType_SP`;
DELIMITER $$
CREATE PROCEDURE `Vodan_Cadastro_ListType_SP`(input  JSON)
BEGIN
...
IF liDados > 0 THEN
	...
	WHILE ind < liDados DO
		...
		IF DSDescricao IS NOT NULL THEN
			...
			ELSE
				SET liCount = (SELECT COUNT(*) FROM tb_listtype
						WHERE  DSDescricao =  DSDescricao
						AND listTypeID != IDListType
						);
		...
		IF  Acao != 'INCLUIR' THEN
			...
			IF IDListType IS NULL THEN
				SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro na Vodan_Cadastro_ListType_SP SQ_ListType não localizado na base';
			END IF;
		END IF;
